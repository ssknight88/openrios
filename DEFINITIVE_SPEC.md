...
|---|---|---|
| Group 0 | ALU0, BRU, DIV; CSR serialized | 3-to-1 MUX |
...
**Inputs** (from frontend / BPU):
- `if_inst_valid[2]` - instruction valid flags (Slot 0, Slot 1)
- `if_inst_pc[2][63:0]` - program counter
- `if_pred_taken[2]` - predicted branch direction
- `if_pred_target_pc[2][63:0]` - predicted target address

**Combinational Admission / Backpressure Check**:
```
ROB_full = (ROB_head == ROB_tail) & ROB_full_flag

slot0_target_group = if_inst_exe_type[0]
slot1_target_group = if_inst_exe_type[1]

slot0_can_dispatch = if_inst_valid[0] &&
                     !ROB_full &&
                     isq_free_for_dispatch[slot0_target_group]

slot1_group_busy_after_slot0 =
    !isq_free_for_dispatch[slot1_target_group] ||
    (slot0_can_dispatch && if_inst_valid[0] && if_inst_valid[1] &&
     (slot0_target_group == slot1_target_group))

slot1_can_dispatch = if_inst_valid[1] &&
                     slot0_can_dispatch &&
                     !ROB_full &&
                     !slot1_group_busy_after_slot0
```
...
**Backpressure Logic**:
- Backpressure is checked at the `ISB -> DSP -> ISQ` admission boundary.
- Admission is determined per slot from that slot's `exe_type`, because `exe_type` selects the destination execution group and therefore the destination ISQ.
...
if (use_rsX == 0):
    ...
    // Read DST_REG first to determine busy state
    if (rsX_is_fp):
        {busy, tag} = FP_DST_REG.read(rsX_idx)
    else:
        {busy, tag} = INT_DST_REG.read(rsX_idx)

    if (!busy && same_cycle_commit_overlay_match):
        // Commit overlay is only visible when DST_REG is NOT busy.
        // If busy==1, a younger in-flight writer exists and the commit
        // overlay's stale value must NOT be used.
        rsX_ready = 1
...
- This overlay has priority over persistent `ARF/DST_REG` state, **but only when `DST_REG[rsX_idx].busy == 0`**.
- **Visibility precondition**: If `DST_REG[rsX_idx].busy == 1`, a younger in-flight instruction has already renamed the same destination. In this case, the commit overlay's value is architecturally stale — the dispatching instruction must wait on the younger producer's tag, not consume the older committed value. The commit overlay is therefore **masked** when `busy == 1`.
- When visible (i.e., `!busy && match`), a matching source resolves as:
```text
rsX_ready    = 1
...
**Bypass bus** is a combinational broadcast from P3 arbiter outputs. No registered CAM, no flip-flop. The comparison is done combinatorially:

**Per source operand per group**:
```verilog
// Combinational comparison (no flip-flop, no posedge clk)
for each entry e in ISQ[group]:
    ...
    // entry_ready = combinational, no registered flip-flop
...
- **New (Bypass)**: P3 arbiter output -> combinational bypass bus -> tag comparator -> `fast_ready` combinatorial -> **same cycle** Select picks entry. Zero delay.

#### 2.3.1.1 rs_ready - Dispatch-Time Static Flag

**What happened to `rs_ready`x** The old CAM path used `rs_ready` as a registered flip-flop (updated at posedge clk when CDB matched). The new Bypass path has no flip-flop for wakeup - it is purely combinational. `rs_ready` is now only set at **dispatch time** (from ARF data, when `busy == 0` in DST_REG), not at wakeup time.
...
**Entry data path**: When an entry has `rs_ready = 0` at dispatch (waiting on dependency), its `rs1_data` field holds `64'h0` (placeholder). The actual data comes from the **FU input MUX** (Section 2.3.2), which routes `bypass_data[g]` directly when `bypass_match` is true.

```verilog
// ISQ entry storage:
entry.rs1_data = 64'h0;    // placeholder, never used for dependency case
entry.rs1_wait_tag = tag0; // only the tag matters

// FU input MUX (two-way at FU input, per source operand):
wire [63:0] rs1_source = entry.rs1_ready   ?  ISQ_sram.rs1_data : // ARF path: data from dispatch
                         bypass_match_rs1  ?  bypass_data[g] :   // Bypass path: combinational from P3
                         64'h0;                                  // placeholder (shouldn't happen)
FU_input.rs1_data = rs1_source;
```

**Data stored in ISQ at dispatch**: When `rs_ready = 1` (no dependence), the ISQ SRAM stores the ARF data read at dispatch. When `rs_ready = 0` (dependence), the ISQ SRAM stores `64'h0` (placeholder). At FU input, a **2-to-1 MUX** per source selects between the stored data (ARF path) and the bypass bus (dependency path):
```verilog
wire [63:0] rs1_source = entry.rs1_ready   ?  ISQ_sram.rs1_data : // No dependence: data from ARF at dispatch
                         bypass_match_rs1  ?  bypass_data[g] :   // Dependence: data from P3 bypass bus
                         64'h0;                                  // Shouldn't happen
```

No `captured_data`, no `bypass_latched_data`, no posedge clk update at wakeup time.
...
- For **non-pipelined** multi-cycle FUs, `fu_busy` must remain `1` throughout:
  - active execution
...
- For **pipelined** multi-cycle FUs, `fu_busy` follows a different contract — see below.

#### 2.3.5 Pipelined vs Non-Pipelined Functional Units

Multi-cycle FUs are classified into two categories based on the `pipelined` flag in the `fu_latency_matrix` configuration:

**Configuration format** (`fu_latency_matrix` in YAML):
```
[FU_name, latency_cycles, pipelined_flag]
```

| FU | Latency | Pipelined | Throughput | Description |
|---|---|---|---|---|
| ALU | 1 | true | 1/cycle | Single-cycle, trivially pipelined |
| BRU | 1 | true | 1/cycle | Single-cycle branch evaluation |
| MUL | 3 | true | 1/cycle | 3-stage Wallace tree multiplier |
| FADDSUB | 3 | true | 1/cycle | FP add/sub pipeline |
| FMUL | 4 | true | 1/cycle | FP multiply pipeline |
| FMA | 4 | true | 1/cycle | Fused multiply-add pipeline |
| FCMP | 3 | true | 1/cycle | FP compare pipeline |
| FMISC | 3 | true | 1/cycle | FP sign-inject, min/max, classify |
| FMV_2I | 3 | true | 1/cycle | FP → INT move |
| FMV_2F | 2 | true | 1/cycle | INT → FP move |
| FCVT_2I | 4 | true | 1/cycle | FP → INT conversion |
| FCVT_I2F | 4 | true | 1/cycle | INT → FP conversion |
| FCVT_F2F | 4 | true | 1/cycle | FP format conversion |
| DIV | 8 | false | 1/8 cycles | Non-pipelined integer divider |
| CSR | 1 | false | serialized | Serialized CSR access |
| FDIV_S | 20 | false | 1/20 cycles | Non-pipelined FP single divider |
| FDIV_D | 23 | false | 1/23 cycles | Non-pipelined FP double divider |
| FSQRT_S | 15 | false | 1/15 cycles | Non-pipelined FP single sqrt |
| FSQRT_D | 18 | false | 1/18 cycles | Non-pipelined FP double sqrt |

**Pipelined FU behavior** (`pipelined = true`):
- The FU accepts a new instruction **every cycle**, regardless of whether previous instructions are still in the execution pipeline.
- `fu_busy` is deasserted on the cycle **after** issue, allowing the ISQ to send a new instruction on the next cycle.
- Multiple instructions may be in-flight simultaneously within the FU pipeline stages.
- Each in-flight instruction independently counts down its `remaining_cycles` and enters writeback arbitration when the count reaches zero.
- **Latency** (cycles from issue to result availability) remains unchanged; only **throughput** improves.

```
Pipelined MUL (latency=3, throughput=1/cycle):

Cycle 0: MUL_A issues → stage 0     fu_busy=0 next cycle
Cycle 1: MUL_B issues → stage 0,  MUL_A → stage 1
Cycle 2: MUL_C issues → stage 0,  MUL_B → stage 1,  MUL_A → stage 2
Cycle 3: MUL_D issues,            MUL_C → stage 1,  MUL_B → stage 2,  MUL_A → writeback
```

**Non-pipelined FU behavior** (`pipelined = false`):
- The FU accepts **at most one instruction** at a time.
- `fu_busy` remains `1` from issue until writeback completion (or flush).
- No new instruction may issue to the same FU slot until the current instruction completes writeback.
- **Latency = Throughput** for back-to-back independent instructions.

```
Non-pipelined DIV (latency=8, throughput=1/8 cycles):

Cycle 0:  DIV_A issues → fu_busy=1
Cycle 1-7: DIV_A executing, fu_busy=1, DIV_B blocked
Cycle 8:  DIV_A writeback → fu_busy=0
Cycle 9:  DIV_B issues → fu_busy=1
```

**Writeback contention**: When a pipelined FU has a result ready for writeback in the same cycle as a non-pipelined FU in the same group, the intra-group arbiter (§2.4.1) resolves contention using static priority. The losing FU's result is held and retried next cycle.
...
head0_can_commit = head0_done && !head0_exception

head1_done       = ROB[head1_tag].done
head1_exception  = ROB[head1_tag].exception_flag
head1_mispredict = ROB[head1_tag].mispredict_flag
head1_can_commit = head1_done && !head1_exception
```

**Commit evaluation order**:
1. Evaluate `head0` first.
2. If `head0_done == 0`, commit nothing and ignore `head1`.
3. If `head0_exception == 1`, commit nothing; Late Flush (Exception) is selected from `head0`.
4. If `head0_mispredict == 1`, commit `head0` only; Late Flush (Mispredict) is selected from `head0`.
5. If `head0_can_commit == 1` and `head0_mispredict == 0`, commit `head0`.
6. Only after `head0` commits may `head1` be evaluated.
7. If `head1_done == 0` or a flush was already selected from `head0`, stop after committing only `head0`.
8. If `head1_exception == 1`, commit `head0` only; Late Flush (Exception) is selected from `head1` in the same cycle.
9. If `head1_mispredict == 1`, commit both `head0` and `head1`; Late Flush (Mispredict) is selected from `head1` in the same cycle.
10. If `head1_can_commit == 1` and `head1_mispredict == 0`, commit both `head0` and `head1`.
...
commit_count_before_flush =
    0 if head0_exception == 1 (flush selected from head0, head0 does not commit)
    1 if head0_mispredict == 1 (flush selected from head0, head0 commits)
    1 if head0 commits normally and head1_exception == 1 (flush selected from head1, head1 does not commit)
    2 if head0 commits normally and head1_mispredict == 1 (flush selected from head1, head1 commits)
...
| `pred_taken` | 1 | Branch prediction context |
| `pred_target_pc` | 64 | Predicted target (for branch resolution) |
...
| Signal | Width | Description |
|---|---|---|
| `commit_valid` | 1 | `ROB[head].done == 1 && (exception_flag == 0)`, ready for ARF writeback (Note: branch mispredict instructions commit normally) |
...
CSR instructions are serialized and do not participate in Group 0 FU writeback arbitration. The Group 0 static priority already defined in Section 2.4.1 covers only `ALU0`, `BRU`, and `DIV`:
`ALU0 > BRU > DIV`
