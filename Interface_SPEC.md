...
**Visibility precondition**: The commit overlay is only usable when `DST_REG[rsX_idx].busy == 0`. If `busy == 1`, a younger in-flight writer has renamed the same destination, and the commit overlay's value is architecturally stale. In this case, P1 must wait on the younger producer's tag rather than consuming the committed value.

```text
commit_overlay_visible =
    commit_overlay_match && !DST_REG[rsX_idx].busy
```

Behavior (when visible):
- resolve the source as ready
- source data comes from `commit_payload[k].result_data`
- `rs_wait_tag = 0`

Priority:
1. for `slot1`, the same-cycle `slot0` rename overlay has highest priority
2. then same-cycle normal P4 commit overlay (**gated by `!DST_REG.busy`**)
...
| `pc` | 64 | ISB_reg | BRU fast path | Original instruction PC kept in the issue payload so BRU/jump logic can avoid a slow metadata-array read. |
| `pred_taken` | 1 | ISB_reg | BRU | Prediction context bit used for mispredict detection. |
| `pred_target_pc` | 64 | ISB_reg | BRU | Prediction target used to compare predicted vs actual target in the BRU. |
...
PC note:
- `pc` is intentionally kept in `ISQ_Payload` for BRU/jump fast-path execution
- `ROB_MetaArray.inst_pc` remains the persistent source for trace, interrupt save-PC, and late-flush recovery
- non-BRU FUs may ignore `pc` even though it is present in the payload
