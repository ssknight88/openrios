...
Else:
    ...
    if !busy && source matches a same-cycle normal P4 commit:
        // Commit overlay only visible when DST_REG is NOT busy.
        // If busy==1, a younger in-flight writer exists; must wait on it.
        rsX_ready = 1
...
Meaning:
- same-cycle commit overlay represents the post-commit architectural state, but is **masked when `DST_REG.busy == 1`** (a younger in-flight writer has renamed the same destination)
...
- then same-cycle normal P4 commit overlay (**gated by `!DST_REG.busy`**)
...
- after serialized CSR execution completes, its real CSR side effect is written into `CSR_PEND_BUF`
