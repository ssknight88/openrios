...
| `G` | Wait for wakeup | `p3_bypass_CDB` → `ISQ` | 不发起 issue。ISQ 对四条 bypass lane 做 tag 比较（`fast_ready_rsX`）：`rsX_wait_tag` 与 `bypass_tag[b]` 相等即命中，命中的源本周期即视为可用。 | 若本周期 entry 因 source 不 ready 不能 issue，命中的 `bypass_data` 在 active edge 被捕获写进 entry 并置 `rsX_ready`（即 `bypass_capture`），当前 entry 继续驻留；下一周期重新检查 source readiness。 |
| `I` | Backpressure | `ISQ` | 本周期不发起 issue，目标 FU 不接收当前 entry。 | 当前 entry 继续由 ISQ 持有，slot 不释放，ownership 不变；下一周期重新检查目标是否可以接收。 |
| `J` | Issue | `ISQ` → `FU_input_mux` → `FU`、`ISQ` → `DSP` | 目标 FU 在本周期接收当前 entry。操作数经 `FU_input_mux` 装配：早已 ready 的源取 entry 里的 `rsX_data`，本拍 `fast_ready_rsX` 命中的源取对应 lane 的 `bypass_data[b]` 直接前递、不落 entry。ISQ issue 与 ISQ release 在同一控制动作中同步完成。 | active edge 释放当前 resident entry；如果 P1 同周期也接受了新的 entry，则新 entry 在同一 edge 写入该 slot，并在下一周期成为 resident entry；否则该 slot 下一周期为空闲。 |
| `K` | Next Cycle | - | - | - |