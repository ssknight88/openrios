...
32 项整数逻辑寄存器重命名表，每项保存 {latest_tag}；执行 P1 分配、P4 已完成 tag-qualified 的 clear 和 Global_Flush_Late 清除，x0 永远保持架构零语义。

## 3. FP tag mapping

32 项浮点逻辑寄存器重命名表，每项保存 {latest_tag}；执行 P1 分配、P4 已完成 tag-qualified 的 clear 和 Global_Flush_Late 清除，模块自身不检查 P3/P4 完成信息。
...
## FU
> 每一个FU type需要支持什么类型，例如: ALU - add, sub
