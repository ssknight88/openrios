# INT_tag_mapping · 32 entry · `{busy, latest_tag}`
...
- **entry 0 硬连 `{busy:0, tag:0}`**：alloc / commit / flush 都不写它，永远读作就绪。
...
- **alloc[s]** = `accept[s]` ∧ `rd_write_enable[s]` ∧ `!rd_is_fp[s]` → `{busy:1, tag}`
  - **同拍 WAW**：两个 slot 写同一格时，保留**年轻**（slot1）的 `tag`
...
- **写口次序**：commit 先 → 同格上更年轻的 alloc 覆盖它 → flush 压掉 alloc，全表回 IDLE
  - 结果是同拍里更年轻的 alloc 赢过 tag 命中的 commit，
      而那条提交在架构上仍然生效
...
- commit（announce，**2 清除口**）
  - broadcast；`commit_tag[k]`(4，k∈{0,1})、`rd_is_fp[k]`(1，k∈{0,1})、`rd_write_enable[k]`(1，k∈{0,1})
    ...
    - 触发；`commit_valid[k]`(1，k∈{0,1}) —— 本拍是否触发合法commit
