...
- **`global_flush_late` 由本模块唯一生成。** 上游（[[CompletionScoreboard微架构文档.md]]
  判定链）只给 `flush_valid` / `flush_tag` / `flush_kind` 
...
```text
cause        = !flush_apply ? 0 :
               (flush_kind == EXCEPTION) ? SCB[flush_tag].exception_cause :
               (flush_kind == INTERRUPT) ? interrupt_cause : 0
is_interrupt = flush_apply ∧ (flush_kind == INTERRUPT)
```
...
```text
redirect_valid = flush_apply
redirect_kind  = flush_apply ? flush_kind : 0
redirect_pc    = !flush_apply ? 0 : case (flush_kind)
...
```text
trap_state_write.valid = flush_apply ∧ (flush_kind != MISPREDICT)
trap_state_write.kind  = flush_apply ? flush_kind : 0
trap_state_write.epc   = flush_apply ? PC_File[flush_tag].inst_pc : 0
trap_state_write.cause = cause
trap_state_write.tval  = (flush_apply ∧ (flush_kind == EXCEPTION)) ? SCB[flush_tag].exception_tval : 0
```
...
- 组合读(in)
    - broadcast；`SCB[flush_tag].mispredict_target_pc`(64)、
      `SCB[flush_tag].exception_cause`、`SCB[flush_tag].exception_tval` —— **1 读口**
