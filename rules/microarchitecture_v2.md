...
**Static Info**：无 fire，不是event，纯信息传递，例如配置（mtvec.MODE）、某状态的对外投影（例如occupancy、be_idle）、普通组合逻辑信号（ARF到ISQ的数据payload）、控制信号（dispatch里的select_payload）
