...
> Source: `work/rtl/rtl_v1/p2p3/ISQ_Group1.sv`  
> Modeling class: single-entry FSM

## 1. Boundary and Responsibility
ALU1 + MUL issue queue. Uses two FU_input_mux instances for bypass selection.
...
| input | `logic FU_ready [G1_NUM_FU]` | event/control |
...
- **Payload定义:** schema follows the listed RTL fields; width/slot cardinality is the declaration shown in Interface.
### `issue` (out)
- **Fire来源:** `isq_valid ∧ operand_ready ∧ FU_ready ∧ ¬global_flush_late`
- **Payload来源:** FU_Group, rs1/rs2_data, imm_*, self_tag, exe_subop
- **Payload定义:** schema follows the listed RTL fields; width/slot cardinality is the declaration shown in Interface.
### `flush` (in)
...
- **Header:** rs1/rs2 ready/wait_tag; FU_Group
- **Payload:** rs1/rs2 data, imm, self_tag, exe_subop
- State is stored only where the RTL has clocked assignments; combinational aliases are views and are not additional state.

## 5. Data Path
dispatch capture -> operand readiness/bypass -> selected FU issue.
