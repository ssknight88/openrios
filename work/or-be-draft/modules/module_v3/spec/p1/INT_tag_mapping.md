...
`INT_tag_mapping`：32-entry integer register rename table（`NUM_GPR=32`、`TAG_W=4`）。
...
### State
#### Per-entry State
1. `IDLE`：对应 integer register 没有未提交 producer。
2. `BUSY`：对应 integer register 由 `entry.tag` 指向未提交 producer。
...
2. `flush`：清除 speculative rename 状态。
   ...
   - State update：`entry.busy[1:31]=0`；`entry.tag` 保持；x0 始终 IDLE。
3. `alloc`：为 integer destination 建立 producer tag。
...
### Header
`entry.tag[0:31]`：`TAG_W=4 bit x 32`；commit_clear 匹配字段；alloc 更新；register 0 固定为 0。
### Payload
无。
...
- `entry.tag/busy[rs_idx[s][x]] -> rename_read[s][x]`：tag/busy；组合读。

## Interface
### In-event
- `alloc[s]`：Notify，`s∈{0,1}`，payload=`self_tag[s]`、`alloc_rd_idx[s]`、integer write qualifiers。
...
### In Static Info
- `rs_idx[s][x]`：5 bit x 4；integer source addresses。
### Out-event
无。
### Out Static Info
- `rename_read[s][x]`：`{tag:4,busy:1}`；`entry.tag/busy[rs_idx[s][x]]`；x0 输出 `{tag:0,busy:0}`。
### Interface Timing
